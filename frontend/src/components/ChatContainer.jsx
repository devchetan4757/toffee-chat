...
const detectMedia = (text) => {
  ...
  if (instaReel) return { type: "instagram_reel", url: instaReel[0] };
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
        ...
        {sortedMessages.length === 0 && <p className="text-center text-xs opacity-50">No messages yet</p>}
            ...
            <div key={message._id} className="chat chat-start group">
              ...
              <div className="chat-bubble max-w-[80%] sm:max-w-[65%] md:max-w-[50%] px-3 py-2">
                ...
                {media ? (
                  ...
                  message.text && <p className="text-xs sm:text-sm leading-snug break-words">{message.text}</p>
                )}
      ...
      {viewImage && (
        ...
        >
          <button className="absolute top-4 right-4 text-white" onClick={() => setViewImage(null)}>
            <X size={22} />
          </button>
          <img
            src={viewImage}
